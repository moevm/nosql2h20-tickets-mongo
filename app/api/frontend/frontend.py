...
from api.frontend.gui_py.main_window import Ui_Dialog as main_window
from api.frontend.gui_py.authorization_window import Ui_Dialog as authorization_window
from api.frontend.gui_py.flights_window import Ui_Dialog as flights_window
from api.frontend.gui_py.registration_window import Ui_Dialog as registration_window


# Вспомогательные функции
def switch_to_admin_page():
    if main_ui:
        main_ui.scrollArea.hide()
        main_ui.scrollArea_2.show()
        _translate = QtCore.QCoreApplication.translate
        main_dialog.setWindowTitle(_translate("Dialog", "TicketBase - Main Window (Administrator)"))


def switch_to_user_page():
    if main_ui:
        main_ui.scrollArea_2.hide()
        main_ui.scrollArea.show()
        _translate = QtCore.QCoreApplication.translate
        main_dialog.setWindowTitle(_translate("Dialog", "TicketBase - Main Window (User)"))
...
if __name__ == "__main__":
    ...
    main_ui = main_window()
    main_ui.setupUi(main_dialog)
    main_ui.scrollArea_2.hide()

    main_ui.auth.clicked.connect(authorization_button)
    main_ui.search.clicked.connect(search_button)
